...
const mostrarTareas = () => {
    ...
    for (let i = 0; i < tareas_obtenidas.length; i++) {
        ...
        <div class="container_list">
                <div class="container_list_1">
                    <input type="checkbox" class="casilla">
                    <p class="actividad">${input}</p>
                </div>
                <div class="container_list_btn">
                    <button class="btn_eliminar" onclick = "eliminarTareas('${input}')"><i class="fas fa-backspace"></i></button>
                </div>
            </div>
        `;
    }
...
btn_todo.addEventListener("click", () => {
    if (input.value === "" || input.value.trim() === "") {
        window.alert("No ha escrito una Tarea");
    } else {
        guardarTareas();
    }
})